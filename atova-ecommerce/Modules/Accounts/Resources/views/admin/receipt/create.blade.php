...
<section class="content">
    ...
    <!-- Default box -->
    @if($mode == 'edit')
        {!! Form::model($receipt, ['method'=>'PUT', 'url' => 'accounts/admin/receipts/' . $receipt->id, 'role'=>'form', 'files'=>true]) !!}
    @else
        {!! Form::open(['method'=>'POST', 'url' => 'accounts/admin/receipts', 'role'=>'form', 'files'=>true]) !!}
    @endif
            ...
            <div class="box box-default">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group {{ $errors->has('payer_id') ? ' has-error' : '' }}">
                        ...
                        {!! Form::select('payer_id', $users, NULL, ['class'=> 'form-control select2', 'placeholder'=>'<-- Select Payer -->']) !!}
                    ...
                    <div class="form-group {{ $errors->has('payment_categories_id') ? ' has-error' : '' }}">
                        {!! Form::label('payment_categories_id', 'Category: ', ['class'=>'control-label']) !!}
                        {!! Form::select('payment_categories_id', $categories, NULL, ['class'=> 'form-control', 'placeholder'=>'<-- Select Category -->']) !!}
                        @if ($errors->has('payment_categories_id'))
                            <span class="help-block">
                                <strong>{{ $errors->first('payment_categories_id') }}</strong>
                            </span>
                        @endif
                    </div>

                    <div class="form-group {{ $errors->has('reason') ? ' has-error' : '' }}">
                        {!! Form::label('reason', 'Receipt reason: ', ['class'=>'control-label']) !!}
                        {!! Form::text('reason', NULL, ['class'=> 'form-control', 'placeholder'=>'Receipt reason']) !!}
